  ...
  export default {
    ...
    methods: {
      ...
      getCalendarCellArray: function (monthNumber, yearNumber) {
        ...
        console.log("start： %d", weekDayCurrent)
        ...
        if (weekDayCurrent !== 1) {
          ...
          for (let i = 0; i < weekDayCurrent - 1; i++) {
            let day = this.makeDT(DateTime.fromObject({
              year: yearNo,
              month: monthNo,
          ...
          console.log(currentWeekArray.reverse())
        }

        for (let thisDateOfMonth = 1; thisDateOfMonth <= currentDay.daysInMonth; thisDateOfMonth++) {
